...
<template>
  <div class="cabinet-explorer">
    <CabinetToolbar
      ...
      :group-field-label="groupFieldLabel"
      :view-mode="viewMode"
      ...
      @update:gridIconSize="gridIconSize = $event"
      @change-sort-field="handleSortFieldChange"
      @change-sort-order="handleSortOrderChange"
      @change-group-field="handleGroupFieldChange"
    ...
    <div class="cabinet-main">
      ...
      <CabinetFilePanel
        ...
        :sorted-filtered-folder-items="sortedFilteredFolderItems"
        :table-columns="tableColumns"
        :selected-id-set="selectedIdSet"
        :clipboard-cut-id-set="clipboardCutIdSet"
        :selection-box="selectionBox"
        :context-menu="contextMenu"
        :can-paste-to-current-folder="canPasteToCurrentFolder"
        :can-paste-to-item-target="canPasteToItemTarget"
        :view-mode="viewMode"
        :grid-icon-size="gridIconSize"
        :sort-field="sortField"
        :sort-order="sortOrder"
        ...
        :property-modal-visible="propertyModalVisible"
        :property-item="propertyItem"
        :is-renaming="isRenaming"
        :build-table-row-event="buildTableRowEvent"
        :build-table-row-class="buildTableRowClass"
        :set-file-panel-ref="setFilePanelRef"
        :set-grid-panel-ref="setGridPanelRef"
        @hide-context-menu="hideContextMenu"
        @blank-contextmenu="handleBlankContextMenu"
        @enter-folder="enterFolderById"
        @grid-blank-mousedown="handleGridBlankMouseDown"
        @grid-order-change="handleGridOrderChange"
        ...
        @open-menu-action="handleOpenMenuAction"
        @copy="handleCopy"
        @cut="handleCut"
        @paste="handlePaste"
        @paste-to-item="handlePasteToItem"
        @rename="handleRename"
        @delete="handleDelete"
...
<script lang="ts" setup>
  ...
  const buildTableRowClass = (record: CabinetItem) =>
    [selectedIdSet.value.has(record.id) ? 'table-row-selected' : '', clipboardCutIdSet.value.has(record.id) ? 'table-row-cutting' : '']
      .filter(Boolean)
      .join(' ');
